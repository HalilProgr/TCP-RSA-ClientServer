...
namespace Impl {
	...
	template<bool SIGNED, typename SomeType>
	class IsNegativeImpl;

	template<typename SomeType>
	class IsNegativeImpl<true, SomeType>
	{
	public:
		bool operator()(SomeType x) { return x < 0; }
	};

	template<typename SomeType>
	class IsNegativeImpl<false, SomeType>
	{
	public:
		bool operator()(SomeType) { return false; }
	};
...
template<typename SomeType>
inline bool isNegative(SomeType x)
{
	using namespace Impl;
	return IsNegativeImpl<std::numeric_limits<SomeType>::is_signed, SomeType>()(x);
}
...
template <typename F, typename SomeType>
inline SomeType& isSafeIntCast(F from)
	/// Returns true if it is safe to cast
	/// integer from F to T.
{
	if (!isIntOverflow<SomeType, F>(from)) return true;
	return false;
}


template <typename F, typename SomeType>
inline SomeType& safeIntCast(F from, SomeType& to)
	/// Returns csted value if it is safe
	/// to cast integer from F to T,
	/// otherwise throws BadCastException.
{
	if (!isIntOverflow<SomeType, F>(from))
	{
		to = static_cast<SomeType>(from);
		return to;
	}
...
template <typename SomeType>
bool strToInt(const char* pStr, SomeType& outResult, short base, char thSep = ',')
	...
	/// the return value is false with the result value undetermined.
{
	...
	if ((base == 10) && (*pStr == '-'))
	{
		if (!std::numeric_limits<SomeType>::is_signed) return false;
		negative = true;
		++pStr;
	}
	else if (*pStr == '+') ++pStr;
	...
	uintmax_t limitCheck = std::numeric_limits<SomeType>::max();
	if (negative)
	{
		poco_assert_dbg(std::numeric_limits<SomeType>::is_signed);
		...
		if (sizeof(SomeType) == sizeof(intmax_t))
			limitCheck = static_cast<uintmax_t>(FPEnvironment::copySign(static_cast<double>(std::numeric_limits<SomeType>::min()), 1));
		else
		{
			intmax_t i = std::numeric_limits<SomeType>::min();
			limitCheck = -i;
		}
	}
	...
	if (negative && (base == 10))
	{
		...
		if (isIntOverflow<SomeType>(i)) return false;
		outResult = static_cast<SomeType>(i);
	}
	else
	{
		if (isIntOverflow<SomeType>(result)) return false;
		outResult = static_cast<SomeType>(result);
	}
...
template <typename SomeType>
bool strToInt(const std::string& str, SomeType& result, short base, char thSep = ',')
...
template <typename SomeType>
bool intToStr (SomeType number, unsigned short base, std::string& result, bool prefix = false, int width = -1, char fill = ' ', char thSep = 0)
